...
static s32 wl_notify_escan_complete(struct wl_priv *wl, struct net_device *ndev, bool aborted, bool fw_abort);
	...
	struct cfg80211_ssid *this_ssid)
{
	...
	if (iscan_req) {
		...
		if (wl->p2p_supported) {
			if (p2p_on(wl) && p2p_scan(wl)&&request&&request->ie) {
...
#if defined (CUSTOMER_HW_ONE) && defined (BCM4335_CHIP)
	maxtxpktglom = 0;
#endif
	bzero(&bssid, sizeof(bssid));
